...
/// Test meta mode equivalence using the same fixtures as single mode.
/// Meta mode is designed for metagenomic sequences and uses pre-computed training files
/// instead of training on the input sequence.
#[test]
fn meta_genome_fixtures_equivalence_summary() {
    if !prodigal_available() {
        eprintln!("Skipping: original prodigal not available in PATH");
        return;
    }

    // Use the same fixtures as single mode - meta mode can process any FASTA
    ...
    for fixture in fixtures {
        ...
        println!("Processing fixture (meta mode): {}", file_name);
    ...
    assert_snapshot!("meta_genomes_gff_summary", summary);
...
/// Test meta mode on individual test files (ecoli.fasta, salmonella.fasta)
#[test]
fn meta_mode_individual_files() {
    if !prodigal_available() {
        eprintln!("Skipping: original prodigal not available in PATH");
        return;
    }

    let test_files = ["tests/data/ecoli.fasta", "tests/data/salmonella.fasta"];
    ...
    for test_file in &test_files {
        ...
        if !path.exists() {
            eprintln!("Skipping missing file: {}", test_file);
            continue;
        }

        let file_name = path.file_name().unwrap().to_string_lossy().to_string();
        println!("Processing (meta mode): {}", file_name);

        let orig_out = NamedTempFile::new().unwrap();
        run_original_prodigal(test_file, orig_out.path().to_str().unwrap(), "gff", "meta").unwrap();

        let rust_out = NamedTempFile::new().unwrap();
        run_orphos(test_file, rust_out.path().to_str().unwrap(), "gff", "meta").unwrap();
        ...
        // Assert high coordinate match for these well-known genomes
        assert!(
            coord_pct >= SIMILARITY_THRESHOLD * 100.0,
            "{}: coordinate match {:.2}% is below threshold {:.2}%",
            file_name,
            coord_pct,
            SIMILARITY_THRESHOLD * 100.0
        );
    ...
    let summary = summary_lines.join("\n");
    assert_snapshot!("meta_mode_individual_files_summary", summary);
}
